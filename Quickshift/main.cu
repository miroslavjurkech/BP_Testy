...
int main(int argc, char ** argv)
{
 ...
 /* cutilCheckError( cutCreateTimer(&totaltimer) );
  ...
  for (int m = 0; m < nmodes; m++) //Only one time
  {
	 ...
	 // }
	  ...
	  /*Miro*/
	  std::cout << "Number of segments: " << count_segments(flatmap, im.N2, im.N1) << std::endl;
	  int *inverted_map = new int[im.N2 * im.N1];
	  for (int i = 0; i < im.N1; i++) {
		  for (int e = 0; e < im.N2; e++) {
			  inverted_map[i * im.N2 + e] = flatmap[e * im.N1 + i];
		  }
	  }
	  save_labels(inverted_map, im.N2, im.N1, argv[2]);
