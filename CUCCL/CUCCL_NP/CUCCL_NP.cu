...
namespace CUCCL {
	...
	__global__ void kernel4(int D_d[], int L_d[], bool* m_d, int N, int width, int height, int flthreshold)
	{
		...
		int threshold = flthreshold * INT_MAX;
		...
		int label = (int)(D_d[id] * INT_MAX); // label <-- L[id]
		int minlabel = N; // N = width * height (max possible label)

						  // Finding minimum connected label 
		bool up = id - width >= 0 && abs(label - (int)(D_d[id - width] * INT_MAX)) <= threshold;
		bool down = id + width < N && abs(label - (int)(D_d[id + width] * INT_MAX)) <= threshold;
		bool left = id % width && abs(label - (int)(D_d[id - 1] * INT_MAX)) <= threshold;
		bool right = id % width + 1 != width && abs(label - (int)(D_d[id + 1] * INT_MAX)) <= threshold;
	...
	__global__ void kernel8(int D_d[], int L_d[], bool* m_d, int N, int width, int height, int flthreshold)
	{
		...
		if (left)
		{
			if (abs(label - (int)(D_d[id - 1] * INT_MAX)) <= threshold)
				minlabel = atom_MIN(minlabel, L_d[id - 1]); //n_id[2]
			if (id - width - 1 >= 0 && abs(label - (int)(D_d[id - width - 1] * INT_MAX)) <= threshold)
				minlabel = atom_MIN(minlabel, L_d[id - width - 1]);//n_id[3]
			if (id + width - 1 < N && abs(label -(int)(D_d[id + width - 1] * INT_MAX)) <= threshold)
	...
	void CCLNPGPU::CudaCCL(int* frame, int* labels, int width, int height, int degreeOfConnectivity, int threshold)
	{
		auto N = width * height;
